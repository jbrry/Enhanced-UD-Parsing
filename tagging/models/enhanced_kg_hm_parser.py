...
@Model.register("enhanced_kg_hm_parser")
class EnhancedKGHMParser(Model):
    """
    A Parser for arbitrary graph structures using a similar scoring model to Kiperwasser and Goldberg (2016).
    Registered as a `Model` with name "enhanced_kg_hm_parser".
    # Parameters
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tag_head = torch.nn.Linear(encoder_dim, tag_representation_dim)
        self.tag_dep = torch.nn.Linear(encoder_dim, tag_representation_dim, bias=False)
        
        num_labels = self.vocab.get_vocab_size("labels")
        
        self.arc_out_layer = Linear(arc_representation_dim, 1, bias=False)
